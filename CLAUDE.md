...
- **Staging index**: plaintext `<hex>\t<path>\n` lines at `.sit/index` (placeholder — migrates to patra once the index grows mutation semantics; see [arch 002](docs/architecture/002-loose-objects-until-patra-bytes.md)). Dedup happens at commit time (last hash per path wins)
