...
8. **dist bundles** — if the change touched any `[lib].modules` source file (`src/util.cyr`, `validate.cyr`, `config.cyr`, `reflog.cyr`, `git_read.cyr`, `git_pack.cyr`, `object_db.cyr`, `index.cyr`, `refs.cyr`, `tree.cyr`, `diff.cyr`, `commit.cyr`, `merge.cyr`, `sign.cyr`, `wire.cyr`, `wire_http.cyr`, `serve.cyr`, `api.cyr`), run `cyrius distlib` and commit the regenerated `dist/sit.cyr` **in the same commit** as the source change. CI's `Verify dist/sit.cyr is in sync` step hard-fails otherwise. **Two profiles now**: `[lib]` → `dist/sit.cyr` (full) and `[lib.read]` → `dist/sit-read.cyr` (the lean read-only bundle for report-only consumers — thoth / owl — via `cyrius distlib read`). A touched file that is ALSO in `[lib.read]`'s subset (everything except `sign` / `wire` / `wire_http` / `serve`) must regenerate **both** — run `cyrius distlib && cyrius distlib read`. (`wire_ssh.cyr` / `wire_https.cyr` are in neither `[lib]` set — transport internals stay out of the consumer bundles.)
9. **Return to step 1.**
