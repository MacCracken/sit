...
- **Source**: `src/main.cyr` — single-file, ~5700 lines. Subcommand dispatch; `init`, `add`, `rm`, `branch`, `checkout`, `tag`, `merge` (`-S` signs), `reset`, `commit` (`-S` signs), `config`, `fsck`, `key`, `verify-commit`, `remote`, `fetch`, `pull`, `push`, `clone`, `log`, `status`, `diff`, `show` (`--stat`), `cat-file`, `owl-file` implemented (24 commands). **v0.5.1 is the file-split refactor** — don't add features to v0.5.1.
