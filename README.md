...
- **Commands** (35): `init`, `add`, `rm`, `mv`, `branch`, `checkout`, `tag`, `merge`, `rebase`, `cherry-pick`, `revert`, `stash`, `merge-base`, `describe`, `reset`, `commit`, `config`, `fsck`, `key`, `verify-commit`, `verify-tag`, `remote`, `fetch`, `pull`, `push`, `clone`, `serve`, `log`, `reflog`, `status`, `diff`, `show`, `blame`, `cat-file`, `owl-file` — see [docs/guides/getting-started.md](docs/guides/getting-started.md).
