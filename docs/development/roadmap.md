...
> **v0.8.x active.** v0.7.x ended at v0.7.6 (2026-05-08, HTTP push + bearer auth + ADR 0007). The originally-slotted v0.7.7 (`dist/sit.cyr` library export + diff-primitive cleanup) and v0.7.8 (SSH) **both moved into v0.8.x** when v0.8.0 took the line opener slot for a toolchain + dep refresh. **v0.8.0 shipped 2026-05-12** — cyrius `5.9.37 → 5.11.34` (100+ patches; load-bearing pickup is cyrius v5.11.33's `PP_IFDEF_PASS` cap raise 2 MB → 8 MB, filed by sit + resolved same-day to clear the sandhi-fold accretion); dep major bumps (sakshi `2.1.0 → 2.2.4`, sankoch `2.1.0 → 2.2.5`, sigil `2.9.3 → 3.1.1`, patra `1.8.3 → 1.9.4`); `lib/` + `src/lib/` + `cyrius.lock` untracked from git; CI gains `Lint` + `Fuzz` steps and a corrected version-pinned toolchain install path. Sandhi's first-party Cyrius TLS arc shipped (v1.3.2 TLS 1.3 0-RTT in `src/tls_policy/`), so ADR 0007's "blocked on first-party Cyrius TLS *existing*" backlog (HTTPS, mTLS) is **unblocked** pending sit-side wire-up and slots into v0.8.x. The v0.6.x perf arc remains the cumulative scoreboard against `add-1MB -48%`, `add-64KB -43%`, `clone -30%`, `log -17%`, `status -9%` from v0.6.0 baseline; no new perf claims this release.
...
- **Stdlib list trimmed.** `bigint` / `ct` / `keccak` removed from `[deps].stdlib` (sigil 3.x transitive resolution covers them via the [deps.sigil] crate). `tls` / `ws` / `http` / `json` kept explicit — cyrius v5.10.x SLOT 19's transitive follow-through still surfaces unresolved enum references at sandhi's TLS 1.3 0-RTT site (`TLS_EARLY_DATA_ACCEPTED`), so the sandhi transitives remain listed pending a follow-on cyrius cycle.
...
| **0.8.1** | **`dist/sit.cyr` library export + diff primitive cleanup.** Was v0.7.7. (1) `[lib]` in `cyrius.cyml` + generated `dist/sit.cyr` so owl (first consumer) can `[deps.sit] modules = ["dist/sit.cyr"]`. Public API namespaced `sit_*`: minimum `sit_repo_open` / `sit_repo_close` / `sit_diff_path(repo, path) → ops_vec` + the existing `ann_*` accessors. ADR 0009 locks the public-API contract (stable vs. internal, semver discipline). (2) Diff primitive correctness: thread `-U<N>` from `cmd_diff` through `group_hunks(annotated, ctx)` (today hardcoded to 3); split `print_file_diff` into pure compute (`sit_diff_path`) + thin print layer (existing stdout writes). All load-bearing primitives (`lcs_diff`, `annotate_ops`, `read_blob_content`, `read_head_tree_entries`, `tree_find`) already exist. | `dist/sit.cyr` (generated), `[lib]` block, ADR 0009 | owl builds against `[deps.sit]` clean; `sit_diff_path` populates owl's VCS marker gutter without `execve`; `cmd_diff -U0` byte-matches `git diff -U0`; new `tests/sit.tcyr` probes assert op counts on a 3-add / 2-mod / 1-del fixture |
| **0.8.2** | **SSH transport.** Was v0.7.8. `ssh user@host -- sit serve --stdio`; length-prefixed framing on stdin/stdout. SSH owns encryption + auth; sit's wire travels over its stdin/stdout — process boundary, not FFI. Heavy fuzz on URL parser (CVE-2017-1000117 host-component injection class). | `src/wire_ssh.cyr`, ADR 0008 | CI sshd loopback clone; `ssh://-oProxyCommand=...` rejected pre-`exec_vec`; round trip works against an OpenSSH server |
| **0.8.3** | **HTTPS via sandhi first-party Cyrius TLS** (client + server). Unblocks the `URL_SCHEME_HTTPS` validator + dispatch path already wired in `validate.cyr` / `wire_transport_check_*`. `_wire_http_post` family gains a TLS handshake step before send/recv; `cmd_serve` adds `--tls-cert <path>` + `--tls-key <path>` flags. **Gate prerequisite**: verify sandhi's TLS surface is actually first-party Cyrius (not a libssl-via-fdlopen shim) — ADR 0007 stays load-bearing. | `src/wire_tls.cyr` (or fold into wire_http.cyr) | end-to-end loopback https:// clone smoke; cert pinning sanity; ADR 0007 verified by reading sandhi's TLS source path |
| **0.8.4** | **mTLS** — client-cert validation, server cert pinning, fingerprint-trust shape. Builds on 0.8.3. | — | mutual-auth smoke against an OpenSSL-generated client cert; spoofed-cert rejection verified |
| **0.8.5+** | **v1 hardening sweep.** Each below is a small-bite v0.8.x patch — slot order TBD; each ships independently with a test gate. Candidates: `receive.denyCurrentBranch` default refuse (close the v0.7.6 footgun); `sit fsck` reachability walk (currently checks integrity but not dangling-object reachability); full `.sitignore` semantics (negation `!pattern`, `**`, char classes `[abc]`, anchored `/foo`); `sit log --graph` (ASCII DAG); `--depth N` shallow clone. | varies | each carries its own gate; collectively they tighten sit's git-parity surface for v1.0.0 |
