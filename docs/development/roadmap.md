...
| **0.8.x** | **Bearer auth over SSH** (belt-and-suspenders) — `_ssh_handle_auth_token` flips from stub to real; `wire_ssh_open`'s capabilities probe detects `"auth":["bearer"]` and loads `~/.sit/serve.token` like the HTTP path. Useful when the SSH session is shared across users (e.g., via a single deploy key) and the server needs per-write authorization beyond ssh's auth. | extends `wire_ssh.cyr` | CI sshd push roundtrip with `--require-auth` on the server side; 401 without token verified |
