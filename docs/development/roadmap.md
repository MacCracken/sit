...
- **`sit fsck --prune`** (`src/object_db.cyr`) completes the v0.8.5 reachability work by removing what it finds. The dangling set is exactly the objects absent from the reachability walk (roots: refs/heads, tags, remotes, detached HEAD, every staged index blob), so reachable history is never touched. New `db_object_delete(db, hex)` helper — `DELETE FROM objects WHERE hash = '<hex>'`, hex validated 64-char before interpolation (never build SQL from unchecked bytes). After deleting, `patra_flush` makes the removal durable across the process exit (the `main` trailer flushes only stdout). Emits `pruned <n> objects`.
- **Safety guard**: refused while `.sit/MERGE_HEAD` exists (in-progress merge), so a half-finished operation's objects can't be dropped.
- **No grace period / no reflog**: immediate and unrecoverable — `git gc --prune=now` semantics, not git's default 2-week grace. Without `--prune`, `fsck` is unchanged (reports dangling, deletes nothing; only integrity errors set a non-zero exit). A reflog-backed grace period is a separate future feature.
- **Verification**: `tests/integration/run.sh` gate — two commits, `reset --hard` to the first → the second's commit/tree/blob dangle; `--prune` reports 3 removed; a **fresh fsck process** sees 0 dangling with the first commit's 3 objects + working tree intact; `--prune` refused under `MERGE_HEAD`. **30 integration assertions** (was 22). 180 unit; lint/fuzz/bench green; dist regenerated; DCE **2.20 MB** flat. No toolchain/dep change.
...
- **`sit log --graph`** (`src/commit.cyr`) — ASCII commit-DAG renderer over the **full** DAG (out+48 parents), unlike the default first-parent log. Position grid (lane *k* at column 2*k*, seams at odd columns): `*` at the emitting lane, `|` elsewhere; merge → `|\` connector, parent-rejoin → `|/` connector; linear degrades to a column of `*`. Reverse-topological, newest-first by timestamp, leftmost-tiebreak → deterministic. Helpers: `print_graph`, `_graph_build_node`, `_graph_lane_find`, `_graph_commit_row`, `_graph_merge_connector`, `_graph_join_connector`, `_graph_write_subject`.
- **`sit clone --depth N`** (`src/wire.cyr`) — shallow clone. `walk_reachable_phased` phase-1 walk gains a depth cap via module globals (`_wire_clone_depth` / `_wire_shallow_boundary`) so the four call sites stay unchanged; kept commits still pull complete tree + blobs. `--depth 1` = exactly 3 objects, `--depth 3` = 9. `.sit/shallow` boundary marker (git's `.git/shallow` parity) lets `sit log` stop cleanly at the boundary; fsck was already boundary-tolerant. Exact for linear history; merge boundary approximate (documented).
- **Bench-fixture refresh** (`tests/sit.bcyr`) — fixed the stale `bench_copy_objects_per_row` (pre-v0.7.3 3-arg `copy_objects` → noise); added the three 0.6.0-scoped Phase-3 groups (LCS diff 100²/1000²/4000²; `is_ignored` 10/50/200 patterns; `hash_blob_of_content` 1 KB/64 KB/1 MB). Baseline `docs/benchmarks/2026-06-13-v0.8.12.md`. `cyrius bench` is a per-release gate again.
- **In-tree integration suite** (`tests/integration/run.sh`) — 19 assertions promoted from `docs/guides/getting-started.md`; new CI step `Smoke — integration suite (v0.8.12)`.
- **Documented simplifications**: `--graph` spacing is sit-native (single space before commit info, not git's lane-padded merge rows); simultaneous open+rejoin in one step is best-effort; `--depth` is clone-only (fetch/pull unlimited; `fetch --deepen` is future). Build / test / lint / fuzz / bench green; 180 unit / 19 integration; dist regenerated; DCE binary **2.20 MB**. No toolchain/dep change.
...
- **Closes the HTTPS slot that was blocked on first-party Cyrius TLS for the whole v0.8.x line.** cyrius 6.x's `lib/tls_native.cyr` (sovereign pure-Cyrius TLS 1.3 on sigil primitives — no fdlopen, no libssl) satisfied ADR 0007's gate. `sit clone https://...` / `sit fetch` work end-to-end against `sit serve --tls`, both ends first-party Cyrius. Read-only this release; https push → v0.8.9.
- **Trust: TOFU / pinned** (ADR 0008 SSH host-key parity). sit pins the peer SPKI SHA-256 (survives cert renewal) in `~/.sit/known_certs`; first connection records, later mismatch refuses. CA + hostname verify is a post-v1 opt-in.
- **Three bites** (committed separately): (1) **TOFU pin store** — new `src/wire_https.cyr` (`known_certs_path`, `_tofu_find_pin` (unit-tested), `_tofu_record_pin`, `tofu_check_pin`). (2) **Client TLS I/O** — `wire_http.cyr` handle carries `is_tls` + host; `wire_http_open` accepts `https://`; per-request `_wire_https_connect` handshake (`set_verify(NONE)` + SPKI pin); `_wire_io_send`/`_wire_io_recv` (TLS-or-plain, 16 KB fragmentation); `wire.cyr` dispatch/`check_readable` accept https. (3) **Server TLS** — `serve.cyr` `sit serve --tls --cert --key` + `_serve_run_tls` (hand-rolled accept loop: `tls_native_new_server` → `server_load_creds` → `tls_native_accept` → `_serve_tls_recv_request` → dispatch → close); 67 response sites routed through `_serve_send_*` wrappers gated on the per-connection `_serve_tls_ctx` global (single-threaded loop → safe; plain http byte-identical). Cert is PEM (sigil `pem_decode_certs`) or DER.
- **Validated e2e**: `sit clone https://127.0.0.1:<port>` against `sit serve --tls` (ECDSA P-256 cert) — TOFU-pinned, all objects over TLS 1.3, fsck-clean, content correct. `tls_native` handshake interops with OpenSSL 3.6.2 (`s_client`/`s_server`). TOFU paths: first-use pins; re-clone matches silently; tampered pin refuses with `SECURITY: certificate pin MISMATCH` (no data transfer).
- **Toolchain / deps**: `tls_native` added to `[deps].stdlib` (no 256-global overflow). Pin `6.1.27 → 6.1.29` (the validated toolchain — the 6.1.27 pin had drifted; the scaffold→working `tls_native` transition in cyrius 6.0.x makes the exact version load-bearing). [ADR 0007](../adr/0007-network-transport-security.md) Update (2026-06-10) records the unblock; cross-repo blocker archived RESOLVED.
- **Out of scope (→ v0.8.9)**: https push (16 MiB body vs 64 KiB TLS buffer needs streaming), keep-alive (drop per-request handshake), socket read-timeout, Ed25519-server-cert `tls_native` gap (ECDSA P-256 works). Build/test/lint/fuzz green; **146 assertions**; DCE binary **2.14 MB**.
...
- **HTTPS (`https://`)** — ✅ **SHIPPED in v0.8.8.** cyrius 6.x's `lib/tls_native.cyr` (sovereign pure-Cyrius TLS 1.3, no fdlopen/libssl; interops with OpenSSL 3.6.2) satisfied [ADR 0007](../adr/0007-network-transport-security.md)'s gate. `sit clone https://` / `fetch` over first-party TLS 1.3, TOFU-pinned. The blocker [`issues/archived/2026-05-13-sandhi-first-party-tls-surface-needed.md`](issues/archived/2026-05-13-sandhi-first-party-tls-surface-needed.md) is archived **RESOLVED**. Read-only; https push is v0.8.9.
...
- ~~**Integration tests in-tree**~~ → **slotted into v0.8.12** (2026-06-13). Promote the shell-level scenarios from `docs/guides/getting-started.md` into `tests/` with fixtures. Current `tests/sit.tcyr` is primitive-assert smoke only, and the bash-based CI smoke steps don't surface failures with the same precision a Cyrius `.tcyr` runner does.
- ~~**Bench fixture refresh**~~ → **slotted into v0.8.12** (2026-06-13). Three bench targets scoped during v0.6.0 but never landed (LCS diff at 100×100 / 1000×1000 / 4000×4000; `_wildmatch` against 10/50/200-pattern `.sitignore` files; `hash_file_as_blob` end-to-end on 1 KB / 64 KB / 1 MB inputs). Landing these makes `cyrius bench` a meaningful per-release gate (run **every** release from 0.8.12 on), restoring the v0.6.x scoreboard for the v0.8.15 closeout.
...
- ~~cyrius 6.2.2 toolchain refresh + dep bumps~~ → **v0.8.11 ✅** (pins-only; shipped 2026-06-13)
- ~~`sit log --graph`~~ → **v0.8.12 ✅** (full-DAG ASCII renderer; shipped 2026-06-13)
