...
- **HTTPS (`https://`)** — gate is sandhi (or cyrius `lib/tls.cyr`) growing a first-party Cyrius TLS surface. Today `lib/tls.cyr` is libssl-via-fdlopen, which [ADR 0007](../adr/0007-network-transport-security.md) forbids sit from consuming. Filed upstream as [`2026-05-13-sandhi-first-party-tls-surface-needed.md`](issues/2026-05-13-sandhi-first-party-tls-surface-needed.md). When that lands, sit's `URL_SCHEME_HTTPS` validator + `wire_transport_check_*` dispatch are already scaffolded (v0.7.1); the wire-up is a single patch at that point.
- **mTLS** — builds on HTTPS. Same gate.
