...
> **v0.8.x active — release 2 of N shipped.** v0.8.0 line opener (2026-05-12, toolchain + dep refresh). **v0.8.1 shipped 2026-05-13** — `dist/sit.cyr` library export (the v0.7.7 originally-slotted work; owl's `execve("git", "diff", ...)` → library-call swap is now unblocked) + diff primitive cleanup (`-U<N>` threading through `cmd_diff` / `cmd_show` / `print_file_diff` / `group_hunks`; pure `compute_file_diff` extracted from `print_file_diff`) + [ADR 0009](../adr/0009-public-api-contract.md) locking the public-API contract (`sit_*` / `ann_*` are stable + SemVer-governed; everything else in the bundle is internal). v0.7.8 (SSH) moves to v0.8.2. Sandhi's first-party Cyrius TLS arc shipped (v1.3.2 TLS 1.3 0-RTT in `src/tls_policy/`), so ADR 0007's "blocked on first-party Cyrius TLS *existing*" backlog (HTTPS, mTLS) is **unblocked** pending sit-side wire-up — slots into v0.8.3+. The v0.6.x perf arc remains the cumulative scoreboard against `add-1MB -48%`, `add-64KB -43%`, `clone -30%`, `log -17%`, `status -9%` from v0.6.0 baseline; no new perf claims this release.
