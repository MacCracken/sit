...
> **v0.8.x active — release 3 of N shipped.** v0.8.0 line opener (2026-05-12, toolchain + dep refresh). v0.8.1 shipped library export + diff primitive cleanup. **v0.8.2 shipped 2026-05-13** — SSH transport (`ssh://`, read-only). Closes ADR 0007's encrypted-over-internet read-side gap via fork+exec on the system's `ssh` binary; SSH owns crypto + auth, sit's wire is the same HTTP/1.1 it speaks over TCP riding the SSH-managed stdin/stdout. [ADR 0008](../adr/0008-ssh-transport.md) pins the process-boundary architecture; CVE-2017-1000117 defense is three-layered (URL whitelist → `_ssh_parse_url` leading-dash check → `--` argv sentinel + no-shell exec). New `sit serve --stdio` mode on the server side; new `src/wire_ssh.cyr` (~530 lines) on the client side; `obj_src` tagged dispatch gains `OBJ_SRC_SSH`. `fuzz_ssh_url_parser` 100K rounds clean; CI sshd-loopback smoke covers both happy-path clone and CVE-injection rejection. **Push over SSH still gated** with `"requires sit 0.8.3+"`. Sandhi's first-party Cyrius TLS arc shipped; HTTPS / mTLS over TLS unblock at v0.8.4+ pending a sandhi-consumable surface. The v0.6.x perf arc remains the cumulative scoreboard against `add-1MB -48%`, `add-64KB -43%`, `clone -30%`, `log -17%`, `status -9%` from v0.6.0 baseline; no new perf claims this release.
...
| **0.8.3** | **Push over SSH.** Mirror of v0.7.6 HTTP push pipeline, layered onto v0.8.2's persistent stdio session. Server-side handlers (`POST /sit/v1/objects/<hex>` + `POST /sit/v1/refs/<refname>` + bearer auth on writes) already exist; only the client side and the `wire_transport_check_writable` gate need to flip. | extend `wire_ssh.cyr` with `ssh_remote_push_object` / `ssh_remote_push_ref`; `_do_push_ssh` in wire.cyr | CI sshd push roundtrip; FF + non-FF rejection; bearer auth over the encrypted-by-ssh channel |
| **0.8.4** | **HTTPS via sandhi first-party Cyrius TLS** (client + server). Unblocks the `URL_SCHEME_HTTPS` validator + dispatch path already wired in `validate.cyr` / `wire_transport_check_*`. `_wire_http_post` family gains a TLS handshake step before send/recv; `cmd_serve` adds `--tls-cert <path>` + `--tls-key <path>` flags. **Gate prerequisite**: verify sandhi's TLS surface is actually first-party Cyrius (not a libssl-via-fdlopen shim) — ADR 0007 stays load-bearing. | `src/wire_tls.cyr` (or fold into wire_http.cyr) | end-to-end loopback https:// clone smoke; cert pinning sanity; ADR 0007 verified |
| **0.8.5** | **mTLS** — client-cert validation, server cert pinning, fingerprint-trust shape. Builds on 0.8.4. | — | mutual-auth smoke against a CA-issued client cert; spoofed-cert rejection verified |
