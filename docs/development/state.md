...
- **Version**: 0.7.3 (read `VERSION` for the authoritative number)
- **Cyrius toolchain**: 5.9.37 (pinned in `cyrius.cyml [package].cyrius`)
- **Binary**: 1.30 MB statically-linked, no dynamic dependencies (DCE; up from 707 KB at v0.7.0 — sandhi opt-in is still the dominant driver, with the v0.7.3 HTTP client adding ~20 KB on top)
- **Status**: **v0.7.x network-transport line — release 4 of N.** v0.7.0 sandhi-fold toolchain unlock; v0.7.1 URL scheme detection + transport dispatch stubs; v0.7.2 read-only HTTP server (`/sit/v1/capabilities`, `/sit/v1/refs`); **v0.7.3 closes the read-only round trip — adds `GET /sit/v1/objects/<hash>` (server) + `src/wire_http.cyr` (470-line client) + the `obj_src` abstraction so `walk_reachable_*` / `copy_objects` run unchanged over either transport.** `sit clone http://127.0.0.1:8484` works end-to-end on a 100-commit fixture: `sit fsck` clean (300/300 objects), 211 ms vs 167 ms for the file:// equivalent (1.26× — the success gate was 3×). Toolchain jumped 5.8.51 → 5.9.37, picking up the cc5_aarch64 cap-propagation fix (issue archived RESOLVED) so aarch64 cross-builds now produce real binaries instead of falling back to the workflow's best-effort swallow. **Next: v0.7.4 — `POST /sit/v1/want` batched object stream**; client falls back to per-object GET if server doesn't advertise `"batch":true`. The v0.6.x perf arc remains the cumulative scoreboard against `add-1MB -48%`, `add-64KB -43%`, `clone -30%`, `log -17%`, `status -9%` from v0.6.0 baseline.
- **Primary target**: Linux x86_64. aarch64 cross-build now produces a real binary under cyrius 5.9.37 (1.45 MB) — the workflow's best-effort swallow is no longer load-bearing
...
16 files total, 8124 lines (up from 15 files / 7372 lines at v0.7.2 — v0.7.3 added `wire_http.cyr` (+530), grew `wire.cyr` to 1087 lines for the `obj_src` abstraction + http branch in `do_fetch` / `cmd_clone`, and grew `serve.cyr` to 349 lines for the `GET /sit/v1/objects/<hash>` handler).
...
| `src/serve.cyr` | 349 | `cmd_serve` + `serve_handle_request` dispatcher; `GET /sit/v1/capabilities` + `GET /sit/v1/refs` (v0.7.2) + `GET /sit/v1/objects/<hash>` (v0.7.3, with `X-Sit-Type: <int>` response header); hand-rolled JSON; loopback-only `--listen` parser |
...
| `src/wire_http.cyr` | 530 | **NEW in 0.7.3.** Sit-side HTTP/1.0 client. `wire_http_open` (URL → 16-byte handle); `_wire_http_request` (recv loop, fl_alloc-backed, growing 64 KiB → 16 MiB); `http_remote_read_refs` + `_wire_http_parse_refs` (v0.7.2 refs JSON parser, validates each ref through `refname_valid` + `hex_prefix_valid`); `http_remote_read_raw` + `http_remote_read_both` (mirrors `db_object_read_raw` / `db_object_read_both` so `obj_src` dispatchers see identical contracts) |
...
| `src/wire.cyr` | 1087 | remote config, reachability (depth-capped) over `obj_src` since v0.7.3 (`OBJ_SRC_DB` / `OBJ_SRC_HTTP` tagged dispatch); `cmd_remote/fetch/pull/push/clone` with `--force-absolute` gate (S-23) + walk-cache (P-04, v0.6.7) + batched transactions (P-03, v0.6.5); `wire_transport_check_readable` / `_writable` split since v0.7.3 (file + http for read; file only for push until v0.7.5); `do_fetch` branches on URL scheme |
