...
- **Version**: 0.6.7 (read `VERSION` for the authoritative number)
- **Cyrius toolchain**: 5.6.35 (pinned in `cyrius.cyml [package].cyrius`)
- **Status**: Fourth v0.6.x perf release shipped. v0.6.4 patra-handle caching → `sit log` -17%. v0.6.5 P-03 batched copy_objects transaction → `sit clone` -15%. v0.6.6 P-10 + P-18 hashmap-backed lookups (algorithmic, no 100-file bench movement, real at scale). v0.6.7 P-04 walk-reachable compressed-bytes cache → `sit clone` -22% (further). Cumulative 0.6.0 → 0.6.7: `log` -16%, `clone` -32%, everything else dep-side bound. Bigger clone wins on patra 1.7.0 `INSERT OR IGNORE` (sit dep bump on that day, no source change). Remaining sit-side: P-17 buffered stdout, P-15 LCS via fl_alloc (hygiene), P-06 smarter decompression sizing. Then v0.7.0 network transport
