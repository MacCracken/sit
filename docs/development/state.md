...
- **Version**: 0.6.10 (read `VERSION` for the authoritative number)
- **Cyrius toolchain**: 5.6.40 (pinned in `cyrius.cyml [package].cyrius`)
- **Status**: v0.6.10 is a dep-bump release: cyrius 5.6.35 → 5.6.40, patra 1.6.0 → 1.8.3. Picks up the `patra_result_get_str_len` sized accessor (closes S-31 — drops sit's `strnlen(s, 256)` workaround). patra 1.7.0 `INSERT OR IGNORE` filed but not consumed (SQL-level only; sit's BYTES-column inserts go through `patra_insert_row` which doesn't expose the flag yet). patra 1.8.x WAL group commit (`PATRA_SYNC_BATCH`) investigated and reverted — durability regression with no perf gain at the bench (sit's `copy_objects` already uses explicit transactions; `cmd_commit` is below the every-64-writes auto-flush threshold; cached handle never closes so pending writes would sit in writeback window). Cumulative 0.6.0 → 0.6.10: `log -18%`, `clone -31%` (unchanged from v0.6.9). Next sit-side mover candidates: explicit transactions + flush-at-exit on multi-insert commands (small effort), or wait on sigil/sankoch/patra-programmatic-OR-IGNORE ships. Then v0.7.0 network transport
