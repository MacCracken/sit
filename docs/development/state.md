...
- **Version**: 0.6.8 (read `VERSION` for the authoritative number)
- **Cyrius toolchain**: 5.6.35 (pinned in `cyrius.cyml [package].cyrius`)
- **Status**: Fifth v0.6.x perf release shipped. v0.6.4 patra-handle caching → `sit log` -17%. v0.6.5 P-03 batched copy_objects → `sit clone` -15%. v0.6.6 P-10+P-18 hashmap-backed lookups (algorithmic, no 100-file bench movement). v0.6.7 P-04 walk-reachable compressed-bytes cache → `sit clone` -22% (further). v0.6.8 P-17 buffered stdout (architectural, no 100-file bench movement, real at scale). Cumulative 0.6.0 → 0.6.8: `log` -17%, `clone` -32%. Remaining sit-side: P-15 LCS via fl_alloc (hygiene only), P-06 smarter decompression sizing (modest, edge case). Sit-side perf at small scale has plateaued — next headline movement requires upstream work (patra 1.7.0 INSERT OR IGNORE, sigil SHA-256 throughput, sankoch zlib throughput, all filed on those repos' roadmaps). Then v0.7.0 network transport
