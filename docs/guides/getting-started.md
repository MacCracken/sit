...
- `sit merge <a> <b> [<c> ...]` — **octopus merge** (1.5.2): folds N branches in and writes one commit with N+1 parents. Refuses to produce conflicts, exactly as git's octopus does — the first conflict aborts with nothing written, because a conflicted N-way merge cannot say which side a marker came from. Merge those pairwise instead
...
- `sit rebase <upstream>` — replay the commits on this branch that aren't on `<upstream>` onto it, oldest first (1.5.3). On conflict it stops with the usual markers; resolve, `sit add`, then `sit rebase --continue`. `sit rebase --abort` restores the branch tip and working tree exactly as they were. ⚠ Rebase **rewrites history** — replayed commits get new oids; the originals stay reachable through the reflog, so `HEAD@{N}` still finds them
- `sit cherry-pick <commit>` — apply that commit's change onto HEAD as a new commit (1.5.1). Runs the same 3-way merge `sit merge` does, with `base = the commit's parent`, `ours = HEAD`, `theirs = the commit`; conflicts write the usual markers plus `.sit/CHERRY_PICK_HEAD`, then resolve + `sit add` + `sit commit`. A pick that changes nothing reports so rather than creating an empty commit
- `sit revert <commit>` — apply that commit's *inverse* as a new commit messaged `Revert "<original>"` (1.5.1). Identical machinery to cherry-pick with `base` and `theirs` swapped, so a revert removes what the commit added — from the working tree as well as the tree
- `sit stash [push|list|pop|drop]` — save tracked working-tree changes and restore HEAD, then bring them back (1.5.1). Entries are real commits: `.sit/refs/stash` names the newest and each carries the previous stash as a second parent, so the stack is the object graph and `sit fsck` protects it. Only tracked files are stashed (git's default); `pop` restores the newest entry and pops it, `drop` discards without restoring
...
- `sit fsck [--prune] [--prune-now]` — three checks, and it **exits non-zero if any of them fails**:
  - **integrity** — each stored object re-hashes to its key (`bad object <hex>`); one that can't be decompressed at all is `unreadable <hex>`
  - **missing referents (1.4.0)** — a commit or tree naming an object the store does **not contain** is reported as `missing <hex>`. Before 1.4.0 an absent object was invisible: only stored objects were enumerated, so the canonical broken repo reported `0 bad, 0 dangling` and exited 0. Shallow clones are exempt — a `--depth N` boundary commit's parents are absent by design and `.sit/shallow` says so
  - **reachability** — objects no ref / index entry points at are `dangling <type> <hex>`; reflog entries count as roots, so a `reset --hard`-discarded tip is protected

  `--prune` removes dangling objects subject to a 90-day grace window (datable commits older than the window; undatable trees/blobs are kept); `--prune-now` is the legacy immediate sweep (`git gc --prune=now`). Both are refused mid-merge, when the store looks corrupt, when anything is `bad`/`missing`, and (1.3.9) when a reachable tree carries entries `parse_tree` can't parse — in each case pruning would delete objects that are still referenced
...
- `sit blame <path>` — per-line last-touch: `<short-oid> (<author> <lineno>) <line>` (1.5.2). Walks first-parent history and attributes each line to the commit that introduced it
...
- **HTTPS trust (1.6.0)** — `https://` uses **TOFU pinning by default**: the first connection records the server's SPKI in `~/.sit/known_certs` and later connections must match. Set `http.sslVerify true` to require a real CA chain *and* a matching hostname instead; `http.caBundle <file>` names a PEM bundle, otherwise the system store is used. When verifying, no TOFU pin is consulted or recorded — a stale pin overriding a CA decision would be a downgrade. `http.sslCert` + `http.sslKey` (both required, 1.6.0) present a client certificate for mTLS
...
- `sit serve <repo> [--listen <addr>:<port>] [--tls --cert <f> --key <f>] [--stdio] [--require-auth]` — host a repo over the `/sit/v1/...` wire protocol (HTTP, HTTPS via first-party TLS 1.3, or SSH stdio). Since 1.6.0 `--listen` accepts any IPv4 literal, but **binding a non-loopback address requires `--tls`** — plain HTTP on a routable interface is refused. Bearer auth (`--require-auth`) stays the operator's call, and now applies over `ssh://` as well as `https://`
...
- HTTPS over public CA certs / mTLS — HTTPS today is TOFU-pinned (CA-chain + hostname verification is a post-v1 opt-in; the **1.7.0** transport-trust minor)
