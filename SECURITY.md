...
sit follows SemVer. Security fixes land on the latest released minor; the most recent `1.x` release is the supported line — upgrade to the latest tag. (Pre-1.0 releases are not separately patched.)
...
- **Read-only `.git/` ingest (1.2.0)** — sit reads existing git repositories read-only (loose objects, packfiles with a self-written OFS/REF delta interpreter + `.idx` v2, refs / `packed-refs`). A malicious `.git/` — crafted packfile, delta chain, or index — is treated as untrusted input: the delta interpreter bounds-checks copy ranges and the result size and caps delta-chain depth, and there is no `.git/` write-back. SHA-1 object ids are read but sit never *computes* SHA-1, so the SHA-256-only trust boundary holds ([ADR 0011](docs/adr/0011-git-read-mode.md) scopes this read-only exception to [ADR 0004](docs/adr/0004-sha256-only.md)).
